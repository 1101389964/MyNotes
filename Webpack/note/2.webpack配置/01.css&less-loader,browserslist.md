...
3. **生成webpack配置文件**
   ...
   1. 建立一个`webpack.config.js`的文件，将配置以commonJS模块化导出，然后直接在命令行执行webpack即可
...
* **配置less-loader**
  ...
  * ```js
    {
        test: /\.less$/,
        //先使用less-loader将less转化为css，再用css-loader解析css，最后用style-loader进行加载
...
```js
{
	...
	use: [
		"style-loader",
		{
            loader: "css-loader",
            options: {
              importLoaders: 2, //因为css-loader前有2个loader，所以importLoaders值为2。less-loader只负责转化为css不负责解析。
        		},
          },
          "postcss-loader",
          "less-loader",
