...
- **construct**
- ![image-20210314150013706](C:\Users\TaoQingAo\AppData\Roaming\Typora\typora-user-images\image-20210314150013706.png)
...
class Star {
    ...
    sing(song) {//方法存放在实例的原型对象上，只有方法存在原型中，而其他的Obj,number,string...都是直接存放在对象上的
      console.log(this.uname + '唱' + song);
    }
}
// 2. 利用类创建对象 new
var ldh = new Star('刘德华', 18);
console.log(ldh); // Star {uname: "刘德华", age: 18}, sing方法保存在对象的原型上面
ldh.sing('冰雨'); // 刘德华唱冰雨
```
...
class Son extends Father{  //这样子类就继承了父类的属性和方法，由于没有使用consructor，所以可以不使用super
}
var damao= new Son('刘');
damao.say();      //结果为 你的姓是刘
```
...
//子类方法存放在原型上，而父类存放在子类实例原型的原型上,所以子类先会执行自己的方法再然后执行父类的方法
  
3. 如果子类想要继承父类的方法,同时在自己内部扩展自己的方法,利用super调用父类的构造函数,super 必须在子类 this 之前调用js
      // 父类有加法方法
      class Father {
        constructor(x, y) {
        this.x = x;
